function [Tw__rr,Tw__rl] = brakeModelRear(reqBrakeTorqueRR,reqBrakeTorqueRL,omega__rr,omega__rl)
    ...
    if (reqBrakeTorqueRR > 0)
        reqBrakeTorqueRR = - reqBrakeTorqueRR;
    end
    
    if (reqBrakeTorqueRL > 0)
        reqBrakeTorqueRL = - reqBrakeTorqueRL;
    end
    ...
    if (abs(reqBrakeTorqueRR) < abs(max_brake_torque_rear))
        Tw__rr = reqBrakeTorqueRR*sin(atan(omega__rr/regularSignScale));
    else
        Tw__rr = -max_brake_torque_rear*sin(atan(omega__rr/regularSignScale));
    end
    
    if (abs(reqBrakeTorqueRL) < abs(max_brake_torque_rear))
        Tw__rl = reqBrakeTorqueRL*sin(atan(omega__rl/regularSignScale));
    else
